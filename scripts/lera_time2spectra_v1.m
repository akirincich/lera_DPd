function lera_time2spectra_v1(base_dir,filein,PC);
...
%tags:
goplot=[1 1];  %plot results of each section
%goplot=[1 0];  %plot results of each section
%goplot=[0 0];  %plot nothing
%%%%%%%%%%%%%%%%


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%establish what the file name of what is coming in looks like infile

%filein='TS_nwt_20171591430.mat';
%filein='TS_nwt_20171692200.mat';
%filein='TS_nwt_20171701130.mat';


%filein='20171701130_nwt.mat';
%filein='20171591430_nwt.mat';

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%

%%%%% get started, load data file, and load configs for this data file
%%% switch to data dir
eval(['cd ' data_dir]);
%%% load the file
load(filein)

%%% load the radar header
eval(['cd ' config_dir])
radar_header;
...
for ichirp=1:file_chirps
    ...
    %do fft
    %    Y=fft(y.*(hamming(MTC)*ones(1,8)));   %do the fft with a hamming window
...
if goplot(2)==1;
    ...
    plot(real(y.*hanning(SpecHead.SamplesPerChirp)));   plot(imag(y.*hanning(SpecHead.SamplesPerChirp)));
...
if goplot(1)==1;
    figure(11); clf;
    subplot(211);  pcolor(20.*real(log10(real(range_data(:,:,1))))); shading flat; colorbar; hg;
    subplot(212);  pcolor(20.*real(log10(imag(range_data(:,:,1))))); shading flat; colorbar;  hg;
end
...
%    something is not right with all parts of the plot
