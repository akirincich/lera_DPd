function [FOreg, FOregi, Alims, HEAD, DN_out]=imageFOLs_v2(data,iant,iFBragg,v_incr,user_param,goplot,HEAD);
...
%
%   Notes:
%   -v1;  adopted from imageFOLs for COS files
...
gains=nan.*ones(n,m,max(iant));  gain3=[];
for i=iant
%    eval(['gains(:,:,i)=10*log10(abs(data.a' num2str(i) num2str(i) ')) + (-40 - 5.8); '])
    eval(['gains(:,:,i)=20*log10(abs(data.a' num2str(i) num2str(i) ')); '])
    %only works with plus...
end
gain3=nanmean(gains(:,:,iant),3); %clear gains
%     figure(4); clf;
% %pcolor(gains(:,:,iant(end))); shading flat; colorbar
...
%    subplot(211); pcolor(gain3); shading flat; colorbar;  caxis([-80 0]); title('Raw Ant3 Spectral Power with segments (red) and sFOLs (white) shown')
